...
function getModList(pages) {
    
    for(let j = 0; j < pages; j++) {
        const promise = got.got(template.replace('NUMBER', j+1)).then(response => {
    ...
    Promise.all(promises).then(() => {
        fs.writeFile("data.json", JSON.stringify(modCards), (error) => {
            ...
            console.log("[INFO] getModList.js: Write to data.json completed.");
            })
...
function getPages() {
    let pages;
    got.got(template.replace("NUMBER", 1)).then(response => {
        const dom = new jsdom.JSDOM(response.body);
